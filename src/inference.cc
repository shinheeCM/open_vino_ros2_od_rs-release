...
void Inference::set_classes(const std::vector<std::string> &class_names) 
{
...
Inference::Inference(const std::string &model_path, const cv::Size &model_input_shape,
	float model_confidence_threshold, float model_NMS_threshold,
	const std::vector<std::string> &class_names)
{
	...
	model_NMS_threshold_ = model_NMS_threshold;
	InitializeModel(model_path);
	set_classes(class_names); // Set the class names
}
